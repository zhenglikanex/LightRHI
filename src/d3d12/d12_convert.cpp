...
namespace light::rhi
{
	const DxgiFormatMapping& GetDxgiFormatMapping(Format abstract_format)
	{
		...
		static constexpr DxgiFormatMapping kFormatMappings[] = {
		...
		static_assert(sizeof(kFormatMappings) / sizeof(DxgiFormatMapping) == size_t(Format::COUNT),
			"The format mapping table doesn't have the right number of elements");

		const DxgiFormatMapping& mapping = kFormatMappings[static_cast<uint32_t>(abstract_format)];
		return mapping;
	}
}
